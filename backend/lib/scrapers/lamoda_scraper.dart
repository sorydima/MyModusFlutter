...
class LamodaScraper extends BaseScraper {
  LamodaScraper({required super.db}) 
      : super(source: 'lamoda', baseUrl: 'https://www.lamoda.ru');
  ...
  Future<List<Product>> _scrapeProducts() async {
    ...
    try {
      ...
      final categories = [
        '/c/355/clothes-zhenskaya-odezhda/',
        '/c/356/clothes-muzhskaya-odezhda/',
        '/c/357/clothes-detskaya-odezhda/',
        '/c/358/clothes-obuv/',
        '/c/359/clothes-aksessuary/'
      ];
      ...
      print('Error in Lamoda scraping: $e');
    } finally {
      await driver.quit();
    }
  ...
  Future<List<String>> _extractProductLinks(selenium.WebDriver driver) async {
    ...
    try {
      ...
      // Get product elements - Lamoda uses different selectors
      final productElements = await driver.findElementsByCss('.product-cards__item');
      ...
      if (links.isEmpty) {
        final altElements = await driver.findElementsByCss('.product-cards__item a');
  ...
  String _extractTitle(parser.Document document) {
    try {
      // Try multiple selectors for Lamoda
      final selectors = [
        '.product-page__title',
  ...
  String _extractCategory(String url) {
    try {
      if (url.contains('/zhenskaya-odezhda')) return 'Женская одежда';
      if (url.contains('/muzhskaya-odezhda')) return 'Мужская одежда';
      if (url.contains('/detskaya-odezhda')) return 'Детская одежда';
